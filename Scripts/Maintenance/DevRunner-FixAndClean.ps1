        ...
        foreach ($pattern in $replacements.Keys) {
            if ($lines[$i] -match [regex]::Escape($pattern)) {
                ...
                if ($DryRun) {
                    ...
                    Add-Content -Path $devLogPath -Value "DRY-RUN: $($file.FullName) Ã¢â€ â€™ $replacement"
                } else {
                    Add-Content -Path $devLogPath -Value "FIXED: $($file.FullName) Ã¢â€ â€™ $replacement"
                }
...
foreach ($dir in $emptyDirs) {
    ...
    if ($DryRun) {
        ...
        Add-Content -Path $cleanupLogPath -Value "DRY-RUN: Empty folder Ã¢â€ â€™ $($dir.FullName)"
    } else {
        Remove-Item -Path $dir.FullName -Force -Recurse
        Write-Host "Deleted empty folder: $($dir.FullName)"
        Add-Content -Path $cleanupLogPath -Value "Deleted: Empty folder Ã¢â€ â€™ $($dir.FullName)"
...
foreach ($file in $duplicates) {
    ...
    if ($DryRun) {
        ...
        Add-Content -Path $cleanupLogPath -Value "DRY-RUN: Duplicate file Ã¢â€ â€™ $($file.FullName)"
    } else {
        Remove-Item -Path $file.FullName -Force
        Write-Host "Deleted duplicate file: $($file.FullName)"
        Add-Content -Path $cleanupLogPath -Value "Deleted: Duplicate file Ã¢â€ â€™ $($file.FullName)"
    }
}
